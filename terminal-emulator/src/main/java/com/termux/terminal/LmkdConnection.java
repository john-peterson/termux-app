   ...
   // //@GuardedBy("mLmkdInputStreamLock")
    ...
    public boolean connect() {
        synchronized (mLmkdSocketLock) {
            ...
            if (socket == null) {
                Log.w(TAG, "Failed to connect to lowmemorykiller, retry later");
                return false;
            }
            ...
            if (mListener != null && !mListener.onConnect(ostream)) {
                Log.w(TAG, "Failed to communicate with lowmemorykiller, retry later");
                IoUtils.closeQuietly(socket);
                return false;
            }
                ...
                synchronized (mReplyBufLock) {
                    if (mReplyBuf != null) {
                        if (mListener.isReplyExpected(mReplyBuf, mInputBuf, len)) {
                            ...
                            mReplyBuf = null;
                            mReplyBufLock.notifyAll();
                            Log.e(TAG, "Received an unexpected packet from lmkd");
                        }
                    } else if (!mListener.handleUnsolicitedMessage(mInputData, len)) {
                        // received asynchronous communication from lmkd
                        // but we don't recognize it.
                        Log.w(TAG, "Received an unexpected packet from lmkd");
                    }
                }
            } catch (IOException e) {
                Log.e(TAG, "Failed to parse lmkd data buffer. Size = " + len);
            }
        }
    }
    ...
    private LocalSocket openSocket() {
        ...
        try {
            ...
            Log.e(TAG, "Connection failed: " + ex.toString());
            return null;
        }
        return socket;
    }
