...
//mClient.logError("scrolll", distanceY+" "+deltaRows+", "+ distanceX+" "+deltaCols);
    ...
    void doScrollX(MotionEvent event, int cols) {
        ...
        //mClient.logError("scrolll","mouse  "+mEmulator.isMouseTrackingActive()+", alt  "+mEmulator.isAlternateBufferActive());
        for (int i = 0; i < amount; i++) {
            if (mEmulator.isMouseTrackingActive()) {
                sendMouseEventCode(event, left ? TerminalEmulator.MOUSE_WHEEL_LEFT : TerminalEmulator.MOUSE_WHEEL_RIGHT, true);
            } else if (mEmulator.isAlternateBufferActive()) {
      /*broken in less let me know if it works elsewhere @john-peterson 
