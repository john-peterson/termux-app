...
//mClient.logError("scrolll", distanceY, distanceX);
    ...
    public void onScreenUpdated(int severity) {
        // try {                                  throw new Exception("blalabla");                                     } catch (Exception e) {             mClient.logStackTraceWithMessage("scroll", "screen updated trace", e);             }
                    mClient.logError("scroll", "view updated BEGIN severity=" + severity + ", row=" + mTopRow + ", old=" + mTopRowOld);
        if (mEmulator == null)
            return;
        ...
        if (isSelectingText() || mEmulator.isAutoScrollDisabled()) {
            ...
            if (-mTopRow + rowShift > rowsInHistory) {
                ...
                severity = 0;
                mTopRow -= rowShift;
                mClient.logError("scroll", " selecting text or auto scroll disabled severity=0, row-=shift=" + rowShift);
